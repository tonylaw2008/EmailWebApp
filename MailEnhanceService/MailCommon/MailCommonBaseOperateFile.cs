...
namespace MailEnhanceService
{
    ...
    { 
        ...
        #region
        public static bool WriteLogNtimes000(string PathFile,string LineContent)
        {
            LineContent = string.Format("[{0:yyyy-MM-dd HH:mm:ss fff}] [{1}]\n", DateTime.Now, LineContent);
            int tryTen = 1; 
            ...
            while(tryTen<10)
            {
                try
                {
                    ...
                    File.AppendAllText(PathFile, LineContent, System.Text.Encoding.UTF8);
                ...
                { 
                    ...
                    if (!System.IO.Directory.Exists(LogFolder))
                    {
                        ...
                        {
                            LogFolder = GetSystemDriveLogPath();
                            string LogPathFileName = Path.Combine(LogFolder,"logger.log");
                            File.AppendAllText(LogPathFileName, string.Format("[{0:yyyy-MM-dd HH:mm:ss}][IOException][WriteLogNtimes] Can not Create BaseDirectory\\log", DateTime.Now), System.Text.Encoding.GetEncoding("UTF-8"));
                            LogFolder = GetSystemDriveLogPath();
                        }
                    }
                    string iOExFile =  string.Format("IOException{0:yyyyMMddHHmmss}.log", DateTime.Now);
                    iOExFile = Path.Combine(LogFolder, iOExFile);
                    LineContent = string.Format("{0} {1}\n",LineContent, e.Message);
                    File.AppendAllText(iOExFile, LineContent, System.Text.Encoding.UTF8);
                    throw e;
                }
                finally
                { 
        ...
        public static bool WriteLogNtimes(string LineContent)
        {
            ...
            //Add to Queue
            MailLoggerQueueHelper.instance.AddData(logger);

            //Get Queue Status
            bool AnalysisStatus = MailLoggerQueueHelper.instance.AnalysisStatus;
            //iterative to handle data
            if (!AnalysisStatus)
            { 
                if (MailLoggerQueueHelper.instance.Getcount() > 0)
                {
                    MailLoggerQueueHelper.instance.PostAnalysis();
                }
            }   
            return true;
        }
